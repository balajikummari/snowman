let query = {
  ...
  whereClause: function (where) {
    ...
    if (
      ...
      Object.keys(where.condition).length !== 0
    ) {
    ...
    if (
      ...
      Object.keys(where.operator).length !== 0
    ) {
      Object.keys(where.operator).map((key) => {
          ...
          if (where.operator[key].length > 0) {
            if (key.toLowerCase() == 'like') {
              if (typeof where.operator[key][1] == 'object') {
                ...
                whereArr.push(
                  where.operator[key][0] +
                    ' ' +
                    key +
                    " '" +
                    where.operator[key][1] +
                    "'"
                )
              }
            } else {
        ...
        // BETWEEN & NOT BETWEEN
          ...
          key.toLowerCase() == 'between' ||
          key.toLowerCase() == 'notbetween'
        ) {
          if (where.operator[key].length > 0) {
            ...
            if (key.toLowerCase() == 'in') {
              whereArr.push(
                fieldName +
                  ...
                  key +
                  ' (' +
                  where.operator[key].join(', ') +
                  ')'
              )
            } else {
              whereArr.push(
                fieldName + ' NOT IN (' + where.operator[key].join(', ') + ')'
              )
            }
          }
        }
          ...
          if (where.operator[key].length > 0) {
            if (key.toLowerCase() == 'gt') {
              if (typeof where.operator[key][1] == 'object') {
                ...
                whereArr.push(
                  where.operator[key][0] + ' > ' + where.operator[key][1]
                )
              }
            } else {
              if (typeof where.operator[key][1] == 'object') {
