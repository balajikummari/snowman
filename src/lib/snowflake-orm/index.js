...
const snowflakeObj = {
  connect: (connectionObj) => {
    return new Promise(function (resolve, reject) {
      new snowflakeConnection(connectionObj).then(
        (data) => {
          dbConnection = data
  ...
  Init: function (table, cols) {
    ...
    this.create = () => {
      let sql = `CREATE TABLE ${this.table} (${helper
        .createTable(this.cols)
        .join(', ')})`
      return new Promise((resolve, reject) => {
        dbConnection.execute({
          sqlText: sql,
    ...
    this.save = (data) => {
      if (
        this.cols.id != undefined &&
        this.cols.id.require != undefined &&
        this.cols.id.require == true
      ) {
        if (data.id == undefined) {
          data.id = uuidv4()
        }
      }
      let sql = `insert into ${this.table} (${helper
        .insertTableColumn('column', data)
        .join(', ')}) select ${helper
        .insertTableColumn('value', data)
        .join(', ')}`
    ...
    this.update = (data, obj) => {
      let sql = `UPDATE ${this.table} SET ${helper
        .basicQuery(data)
        .join(', ')} ${helper.where(obj)}`
      console.log(sql)
      return new Promise((resolve, reject) => {
        dbConnection.execute({
          sqlText: sql,
