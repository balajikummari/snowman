...
let helper = {
  ...
  basicQuery: (cols) => {
    ...
    Object.keys(cols).map((key) => {
      if (typeof cols[key] == 'string') {
        array.push(key + "='" + cols[key] + "'")
      } else if (typeof cols[key] == 'object') {
        array.push(key + '=' + `parse_json('${JSON.stringify(cols[key])}')`)
      } else {
        array.push(key + '=' + cols[key])
      }
    })
  ...
  where: (condition) => {
    if (
      ...
      Object.keys(condition.where).length !== 0
    ) {
  ...
  insertTableColumn: (type, cols) => {
    ...
    Object.keys(cols).map((key) => {
      ...
      if (typeof cols[key] == 'string') {
        ...
        value.push(`parse_json('${JSON.stringify(cols[key])}')`)
      } else {
        value.push(cols[key])
      }
    })
  ...
  query: (table, condition) => {
    ...
    if (
      ...
      Object.keys(condition.order).length !== 0
    ) {
    ...
    // Limit & Offset
    if (
      (typeof condition.limit == 'object' && condition.limit.length > 0) ||
      typeof condition.limit == 'number'
    ) {
  ...
  queryFunction: (table, condition) => {
    ...
    // Display Column List based on Condition. Otherwise All Column will be display
    if (
      Array.isArray(condition.functions) == true &&
      ...
      for (let i = 0; i < condition.functions.length; i++) {
        funcArr.push(
          functions(
            condition.functions[i].name,
            condition.functions[i].option
          ).join(', ')
        )
      }
  ...
  join: (table, joinType, condition) => {
    ...
    for (let i = 0; i < condition.include.length; i++) {
      for (let c = 0; c < condition.include[i].column.length; c++) {
        columnList.push(
          condition.include[i].model.table +
            '.' +
            condition.include[i].column[c]
        )
      }
      tableList.push(condition.include[i].model.table)
      eqList.push(
        condition.include[i].model.table + '.' + condition.include[i].eqColumn
      )
    }
